...
@Component({
  selector: 'app-pocfgreen02',
  templateUrl: './pocfgreen02.page.html',
  styleUrls: ['./pocfgreen02.page.scss'],
})
export class Pocfgreen02Page implements OnInit {
  ...
  loaddata(padding: number, infiniteScroll?){
    ...
    this.sub = this.poSv
    .getcfgreen('readid',this.ionicForm.value)
    .subscribe((data) => {
      if (data !== null) {
   ...
   const confirm =  await this.alertCtrl.create({
     ...
     },
     {
       text: 'ตกลง',
         handler: (data: any) => {
         this.sub = this.poSv
         .crudcfgreen(this.ionicForm.value,'insert')
         .subscribe((data) => {
           if (data !== null) {
               if(data.status === 'ok'){
   ...
   dataall.forEach( array1Ttem => {
       dataselect.forEach( array2Item => {
          if(array1Ttem.assign_id == array2Item.assign_id){
             ...
             array1Ttem.poassign_green  = 1;
             array1Ttem.poassign_greentxt  = 'งานปักป้ายเขียวสำเร็จ';
         }
       });
     });
 }

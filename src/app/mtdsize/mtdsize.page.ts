...
@Component({
  selector: 'app-mtdsize',
  templateUrl: './mtdsize.page.html',
  styleUrls: ['./mtdsize.page.scss'],
})
export class MtdsizePage implements OnInit {
  ...
  submitForm(){
    ...
    if (!this.ionicForm.valid) {
      ...
      this.sub = this.mtdSv
      .crudmtdsize(this.ionicForm.value,typesql)
      .subscribe((data) => {
        if (data !== null) {
          if (typesql === "insert") {
            if(data.status === 'ok'){
              this.data.unshift({
                id: data.id,
                size: this.ionicForm.controls.size.value,
                highlight: true,
              });
            }

...
@Component({
  selector: 'app-mtdproductdetail',
  templateUrl: './mtdproductdetail.page.html',
  styleUrls: ['./mtdproductdetail.page.scss'],
})
export class MtdproductdetailPage implements OnInit {
  ionicForm: FormGroup;isSubmitted = false; 
  sub: Subscription;filterTerm: string; 
  page = 0;maxpadding = 0;limit = 50;
  id: number;data = [];
  portControl: FormControl; ports: any;
  product_id:number;product_name:string;product_pattern:string;product_model:string;product_color:string;
  constructor(private navCtrl: NavController, private activatedRoute: ActivatedRoute,public formBuilder: FormBuilder,
    ...
    private alertCtrl: AlertController) { 
      this.activatedRoute.queryParams.subscribe((res) => {
        ...
        //console.log(item);
        this.product_id = item[0]['id'];
        this.product_name = item[0]['product_name'];
        this.product_pattern = item[0]['product_pattern'];
        this.product_model = item[0]['product_model'];
        this.product_color = item[0]['product_color'];
        //console.log(res['xxx']);
      });
    }
  ...
  submitForm(){
    ...
    if (!this.ionicForm.valid) {
     ...
     this.sub = this.mtdSv
      .crudmtdproductdetail(this.ionicForm.value,typesql)
      .subscribe((data) => {
        if (data !== null) {
          if (typesql === "insert") {
            if(data.status === 'ok'){
  ...
  loaddata(padding: number, infiniteScroll?) {
    let datalimit;
    this.sub = this.mtdSv
      .getmtd(8,padding,this.limit,this.product_id)
      .subscribe((data) => {
        if (data !== null) {
    ...
    const confirm =  await this.alertCtrl.create({
      ...
      },
      {
        text: 'ตกลง',
          handler: (data: any) => {
           if(data['cause']){
            this.sub = this.mtdSv.crudmtdproductdetail(item, 'cancel',data['cause']).subscribe(
              (data) => {
                if(data.status == 'ok')
                {   
                  this.configSv.ChkformAlert(data.message);
